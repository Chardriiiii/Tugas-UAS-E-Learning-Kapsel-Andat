...
class InteractionLogModel(Base):
    ...
    # PERBAIKAN: Boleh NULL (None), Default None
...
class StopActivityRequest(BaseModel):
    # PERBAIKAN: Optional, default None. Tidak wajib diisi.
...
class LogResponse(BaseModel):
    ...
    # PERBAIKAN: Score bisa null di response
    score: Optional[float] = None 
...
def start_activity(data: StartActivityRequest, db: Session = Depends(get_db)):
    ...
    is_enrolled = db.query(EnrollmentModel).filter(
        ...
        EnrollmentModel.course_id == act.course_id
    ).first()

    if not is_enrolled:
        raise HTTPException(status_code=403, detail="Akses Ditolak: Anda belum mengambil mata kuliah ini.")
    ...
    new_log = InteractionLogModel(
        ...
        status="in_progress",
        score=None # Default None saat mulai
    )
    db.add(new_log)
    db.commit()
    db.refresh(new_log)
...
def stop_activity(id: int, req: StopActivityRequest, db: Session = Depends(get_db)):
    ...
    # PERBAIKAN LOGIKA:
    # Jika user mengirim score (misal untuk Quiz), kita simpan.
    # Jika user TIDAK mengirim score (untuk Video), biarkan tetap None.
...
def get_logs(student_id: str = None, db: Session = Depends(get_db)):
    ...
    if student_id:
        query = query.filter(InteractionLogModel.student_id == student_id)
    return query.all()